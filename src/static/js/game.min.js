import{addNotification as e}from"./notification.js";document.addEventListener("DOMContentLoaded",()=>{let t=io("/game"),s=document.getElementById("visitors"),n=document.getElementById("surrended"),a=document.getElementById("current-player"),r=document.getElementById("timer"),l=document.getElementById("mobile-timer"),i=document.getElementById("timer-progress-circle"),o=document.getElementById("mobile-timer-progress"),d=document.getElementById("selected-char"),c=document.getElementById("btn-surrend"),u=document.getElementById("players"),m=document.getElementById("play-mode-switch"),p=document.getElementById("play-mode-label");document.getElementById("board-container");let $=document.getElementById("board-viewport"),g=document.getElementById("board-wrapper");document.getElementById("game-board");let f=document.getElementById("zoom-slider"),y=document.getElementById("zoom-percentage"),v=document.getElementById("zoom-in"),h=document.getElementById("zoom-out"),b=document.getElementById("reset-zoom"),_=document.getElementById("pan-indicator"),E=null,L=0,x=0,B=!0,I=100,T=!1,w=0,C=0,k=0,A=0;function S(){f.addEventListener("input",e=>{I=Number.parseInt(e.target.value),z()}),v.addEventListener("click",()=>{I=Math.min(200,I+10),f.value=I,z()}),h.addEventListener("click",()=>{I=Math.max(50,I-10),f.value=I,z()}),b.addEventListener("click",()=>{I=100,f.value=I,z(),$.scrollLeft=0,$.scrollTop=0}),$.addEventListener("wheel",e=>{if(e.ctrlKey||e.metaKey){e.preventDefault();let t=e.deltaY>0?-10:10;I=Math.max(50,Math.min(200,I+t)),f.value=I,z()}}),$.addEventListener("mousedown",D),$.addEventListener("mousemove",Y),$.addEventListener("mouseup",P),$.addEventListener("mouseleave",P),$.addEventListener("touchstart",N,{passive:!1}),$.addEventListener("touchmove",H,{passive:!1}),$.addEventListener("touchend",P),$.addEventListener("mouseenter",()=>{I>100&&(_.style.opacity="1")}),$.addEventListener("mouseleave",()=>{_.style.opacity="0"})}function z(){let e=I/100;g.style.transform=`scale(${e})`,y.textContent=`${I}%`,I>100?_.style.display="block":_.style.display="none",h.disabled=I<=50,v.disabled=I>=200}function D(e){I<=100||(T=!0,$.style.cursor="grabbing",w=e.pageX-$.offsetLeft,C=e.pageY-$.offsetTop,k=$.scrollLeft,A=$.scrollTop,e.preventDefault())}function N(e){if(I<=100||1!==e.touches.length)return;T=!0;let t=e.touches[0];w=t.pageX-$.offsetLeft,C=t.pageY-$.offsetTop,k=$.scrollLeft,A=$.scrollTop,e.preventDefault()}function Y(e){if(!T||I<=100)return;e.preventDefault();let t=e.pageX-$.offsetLeft,s=e.pageY-$.offsetTop,n=(t-w)*2,a=(s-C)*2;$.scrollLeft=k-n,$.scrollTop=A-a}function H(e){if(!T||I<=100||1!==e.touches.length)return;e.preventDefault();let t=e.touches[0],s=t.pageX-$.offsetLeft,n=t.pageY-$.offsetTop,a=(s-w)*2,r=(n-C)*2;$.scrollLeft=k-a,$.scrollTop=A-r}function P(){T=!1,$.style.cursor=I>100?"grab":"default"}function j(e){X(),x=((L=1e3*e)-Date.now())/1e3,E=setInterval(()=>{let e=L-Date.now();M(Math.max(0,e/1e3)),e<=0&&(clearInterval(E),t.emit("timeout"))},100)}function M(e){let t=e.toFixed(1);if(r&&(r.innerText=t),l&&(l.innerText=t),i&&x>0){let s=e/x;i.style.strokeDashoffset=s,s>.5?i.style.stroke="#28a745":s>.25?i.style.stroke="#ffc107":i.style.stroke="#dc3545"}if(o&&x>0){let n=e/x*100;o.style.width=`${n}%`,n>50?o.style.backgroundColor="#28a745":n>25?o.style.backgroundColor="#ffc107":o.style.backgroundColor="#dc3545"}}function X(){E&&clearInterval(E)}function G(e){let t=u.getAttribute("data-current-user");a.innerText=e,e==t?a.classList.add("bg-success"):a.classList.remove("bg-success"),O(e)}function O(e){document.querySelectorAll(".player-score-card").forEach(e=>{e.classList.remove("current-player-highlight")});let t=document.getElementById(`item-${e}`);t&&t.classList.add("current-player-highlight")}function R(e,t){m.disabled=!t,m.checked=e,p.innerHTML=e?`
